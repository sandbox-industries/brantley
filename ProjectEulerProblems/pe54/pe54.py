...
class Hand:
    ...
    def find_hand_rank(self):
        if not self.consecutive and not self.same_suit:
            return self.how_many_of_a_kind()

        return max(self.how_many_of_a_kind(), self.straight(), self.flush(), self.straight_flush(), self.royal_flush())
